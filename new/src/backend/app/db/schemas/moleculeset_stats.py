...
class AtomTypeCount(Base):
    ...
    molecule_set_id = mapped_column(sa.VARCHAR(100), sa.ForeignKey("molecule_set_stats.file_hash"))
    molecule_set_stats = relationship("MoleculeSetStats", back_populates="atom_type_counts")

    def __repr__(self):
        return f"""<AtomTypeCount
            id={self.id},
            symbol={self.symbol},
            count={self.count},
...
class MoleculeSetStats(Base):
    ...
    def __repr__(self):
        return f"""<MoleculeSetStats
            file_hash={self.file_hash},
            total_molecules={self.total_molecules},
            total_atoms={self.total_atoms},
            atom_type_counts={self.atom_type_counts}"""
